...
export default function CategoryPageHeader({ category , totalSemanal, totalMensual}) {
    ...
    const GastoSemanal = () => {
        return <div className="flex-1 h-full flex flex-col justify-evenly items-center">
            <h3 className="text-center">Últimos 7 días:</h3>
            <h3 className={`${textResponsive}`}>${totalSemanal ?? '---'}</h3>
        </div>
    }
    const GastoMensual = () => {
        return <div className="flex-1 h-full flex flex-col justify-evenly items-center">
            <h3 className="text-center">Últimos 30 días:</h3>
            <h3 className={`${textResponsive}`}>${totalMensual ?? '---'}</h3>
        </div>
    }
    const textResponsive = "text-xl sm:text-2xl md:text-3xl lg:text-3xl"
