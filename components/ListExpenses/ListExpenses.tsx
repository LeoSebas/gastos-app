...
export default function ListExpenses(props) {
    ...
    const handleClickModifyExpense = (_id) => {
        const currentExpense = results?.expenses?.find((e) => {
            return e._id === _id
        })
    ...
    const handleClickDeleteExpense = (_id) => {
        setSelectedExpense(results?.expenses?.find((e) => {
            return e._id === _id
        }))
        ...
        <div className={style.ListExpenses}>
            <div className={style.ListExpenses__list}>
                ...
                {results?.expenses?.map((expense) => <ExpenseRow
                    ...
                    color={getCategory(expense.categoryID).color} handleModify={handleClickModifyExpense}
                    handleDelete={handleClickDeleteExpense}/>)}
            </div>
            <Dialog open={showModifyExpense} aria-labelledby="parent-modal-title"
                ...
                <ExpenseForm action={ExpenseFormAction.modify} expense={selectedExpense} dismiss={handleClose}
                             handleReload={reloadExpenses}/>
            </Dialog>
            <Dialog open={showDeleteExpense} aria-labelledby="parent-modal-title"
                    aria-describedby="parent-modal-description" onClose={handleClose}>
                <DeleteExpenseDialog expense={{expenseID: selectedExpense?._id}} handleClose={handleClose}
                                     handleReload={reloadExpenses}/>
            </Dialog>
        </div>
    )
